  ...
  real partial_sum_fullddm(array[] real rt_slice, int start, int end,
    ...
    array[] int resp, array[] int cnd) {
      ...
      for (i in start:end) {
        if (resp[i] == 1) {
          // upper threshold
          ans += wiener_full_lpdf(rt_slice[i+1-start] | a, v_m[cnd[i]], zr_m, t0_m, v_s,  zr_s, t0_s);
        } else {
          // lower threshold (mirror drift and starting point!)
          ans += wiener_full_lpdf(rt_slice[i+1-start] | a, -v_m[cnd[i]], 1 - zr_m, t0_m, v_s,  zr_s, t0_s);
        }
      }
      return ans;
    }
...
model {
  ...
  if (parallel) {
    ...
    for (i in 1:N) {
      if (resp[i] == 1) {
        // upper threshold
        target += wiener_full_lpdf(rt[i] | a, v_m[cnd[i]], zr_m, t0_m, v_s,  zr_s, t0_s);
      } else {
        // lower threshold (mirror drift and starting point!)
        target += wiener_full_lpdf(rt[i] | a, -v_m[cnd[i]], 1-zr_m, t0_m, v_s,  zr_s, t0_s);
      }
    }
  }
}
